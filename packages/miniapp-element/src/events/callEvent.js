    ...
    EventTarget.$$process(originNode, eventName, evt, extra, (domNode, evt, isCapture) => {
        ...
        setTimeout(() => {
            ...
            if (domNode.tagName === 'A' && evt.type === 'click' && !isCapture) {
                ...
                if (targetDomNode.tagName === 'INPUT') {
                    ...
                    if (type === 'radio') {
                        ...
                        for (const otherDomNode of otherDomNodes) {
                            if (
                                otherDomNode.type === 'radio' && otherDomNode !== targetDomNode
                            ) {
                ...
                (domNode.tagName === 'BUTTON' || domNode.tagName === 'BUILTIN-COMPONENT' && domNode.behavior === 'button') &&
                evt.type === 'click' &&
                !isCapture
            ) {
                // 处理 button 点击
                ...
                if (type === 'submit') {
                    ...
                    if (sliderList.length) {
                        sliderList.forEach(
                            item => formData[item.getAttribute('name')] = +item.getAttribute('value') || 0
                        )
                    }
