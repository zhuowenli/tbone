...
test('event', () => {
    ...
    const miniappEvent = {timeStamp: Date.now}
    const seqList = []
    ...
    target = e
    expect(EventTarget.$$process(target, 'click', miniappEvent)).toBeInstanceOf(Event)
    ...
    expect(EventTarget.$$process(c, 'click', miniappEvent)).toBeInstanceOf(Event)
