...
export default function filterNodes(domNode, level, component) {
    ...
    return childNodes
        .map((child) => {
            ...
            // Check child nodes
            domInfo.isLeaf = !domInfo.isImage &&
                domInfo.type === 'element' &&
                !child.children.length &&
                NEET_RENDER_TO_CUSTOM_ELEMENT.indexOf(child.tagName.toUpperCase()) === -1;

            if (domInfo.isLeaf) {
                domInfo.content = child.childNodes
                    .map((childNode) => (childNode.$$domInfo.type === 'text' ? childNode.textContent : ''))
                    .join('');
            }

            // Check simple node that can be rendered as view
            domInfo.isSimple = !domInfo.isImage &&
                !domInfo.useTemplate &&
                !domInfo.isLeaf &&
                domInfo.type === 'element' &&
